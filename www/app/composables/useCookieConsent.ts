 ...
 *
 * Opt-in model: Google Analytics (nuxt-gtag, configured `initMode: 'manual'`)
 * never loads until the visitor accepts here. The choice lives in localStorage
 * rather than a cookie, so the site keeps setting no first-party cookies of its
 * own — the privacy policy says as much. A stored preference is strictly
 * necessary and therefore exempt from consent itself.
 *
...
const STORAGE_KEY = 'pasdiu:analytics-consent'
